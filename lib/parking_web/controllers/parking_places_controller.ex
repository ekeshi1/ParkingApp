defmodule ParkingWeb.Parking_placeController do
  ...
  def create(conn, %{"parking_place" => parking_place_params}) do
    query = from t in Parking_place, where: t.busy_places < t.total_places, select: t
    ...
    places =  places |> Enum.map(fn x ->
      ...
      Map.merge( %{hour_price: hour_price, realtime_price: realtime_price,hour_rate: zone.hourly_rate, realtime_rate: zone.realtime_rate},x)
      end  ) |> Enum.sort(&(&1.distance< &2.distance))

    Enum.each(places, fn(place) ->          changeset=Parametre.changeset(%Parametre{}, %{address: place.address, busy_places: place.busy_places , total_places: place.total_places, distance: place.distance, hour_price: place.hour_price, name: place.name, realtime_price: place.realtime_price, zone_id: place.zone_id, hour_rate: place.hour_rate, realtime_rate: place.realtime_rate})
